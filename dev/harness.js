...
(function () {
     ...
     *
     * `host.js` builds `parameters` from its `inputs` option alone, so a
     * declared property with a manifest default arrives as `undefined` here and
     * reading `.raw` off it throws — while the same read works on a form,
     * because the platform applies the default. The fix belongs here rather
     * than as a `?.` in the control: a control is entitled to assume a declared
     * property exists, and coding around the gap would hide the day one stops
     * being supplied.
     */
    ...
    function options() {
        return {
            ...
            device: document.getElementById('harness-device').checked,
            inputs: Object.assign({}, MANIFEST_DEFAULTS, {
                showMap: document.getElementById('harness-showmap').checked,
            }),
